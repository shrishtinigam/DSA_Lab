...
*   front of the linked list, and if the we come across a node less than x, it is moved to the end of 
*   linked list.
...
void separateByX(DoublyLinkedList * dll, int x)
{
    ...
    while(ptr != NULL)
    {
        ...
        printf("%d\n", ptr->data);
        ptr0 = ptr0->next;
        if(ptr->data >= x) // Move to start
        {
            if(ptr->prev == NULL) // Already at the start
                continue;
            ptr->prev->next = ptr->next;
            ptr->next->prev = ptr->prev;
